...
* payload.type ^slicing.discriminator.type = #value
* payload.type ^slicing.discriminator.path = "coding.system"
* payload.type ^slicing.rules = #open
...
* payload.type contains
    ...
    formatCode 0..*
* payload.type[meldingensFunksjon] from urn:oid:2.16.578.1.12.4.1.1.8279 (required)
* payload.type[meldingensFunksjon] ^definition = "no-basis: for ebXML and AMQP messages in Norway one is expected to list all mesage types that the endpoint supports using the CodeSystem Meldingens funksjon (OID=8279). Version information (version of the payload document) shold be given in the coding.version element.\r\n\r\nThe payload type describes the acceptable content that can be communicated on the endpoint."
* payload.type[formatCode] from urn:oid:2.16.578.1.12.4.1.1.0000 (required)
* payload.type[formatCode] ^short = "Specific information about payload type and version"
* payload.type[formatCode] ^definition = "no-basis: Specific information about payload type and version. Information about the payload type and version for ebxml and amqp messages\r\n\r\nThe payload type describes the acceptable content that can be communicated on the endpoint."
* payload.mimeType from $bcp13.txt (required)
* payload.mimeType ^definition = "no-basis: should we list relevant codes for ebXML messages? List appropriate media types for FHIR REST endpoints?\r\nXML: application/fhir+xml\r\nJSON: application/fhir+json\r\nRDF: application/fhir+turtle\r\nebXML messages: text/xml\r\n\r\nThe mime type to send the payload in - e.g. application/fhir+xml, application/fhir+json. If the mime type is not specified, then the sender could send any content (including no content depending on the connectionType)."
